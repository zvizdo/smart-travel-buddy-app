...
| `RouteService` | **In `shared/shared/services/`**. Google Routes API v2. Constructor accepts optional `flight_service: FlightService` — wired at startup in both backend and MCP server. `get_route_data(from, to, mode, departure_time?)` -> `RouteData(polyline, travel_time_hours, distance_km)`. Handles drive/transit/walk via Routes API, flights via airport IATA resolution + FlightService (returns duration + haversine distance, no polyline), ferry returns None. `fetch_and_patch_route_data()` always writes `route_updated_at` to the edge (including on failure) so the frontend can clear the recalculating shimmer. Sends `languageCode: "en"` always; sends `departureTime` + `TRAFFIC_AWARE` for DRIVE/TRANSIT when departure_time is provided. |
...
- **`time_inference.py`**: `enrich_dag_times()` — pure forward-only topological enrichment fills `arrival_time` / `departure_time` / `duration_minutes` for flex nodes, flags `arrival_time_estimated` / `timing_conflict` / `overnight_hold`.
