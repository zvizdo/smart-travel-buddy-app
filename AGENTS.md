...
| `RouteService` | **In `shared/shared/services/`**. Google Routes API v2. `get_route_data(from, to, mode, departure_time?)` -> `RouteData(polyline, travel_time_hours, distance_km)`. Sends `languageCode: "en"` always; sends `departureTime` + `TRAFFIC_AWARE` for DRIVE/TRANSIT when departure_time is provided. |
